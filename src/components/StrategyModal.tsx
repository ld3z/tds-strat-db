...
const StrategyModal: React.FC<StrategyModalProps> = ({ strategy, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 rounded-2xl border border-slate-700 max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden shadow-2xl shadow-sky-500/10">
        {stratImgUrl && (
          ...
          <div className="flex items-end justify-between">
            <div className="flex-1">
              <div className="flex items-center space-x-3">
                <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-blue-500">
                  {strategy.title}
                </h2>
                {strategy.starred && (
        ...
        <div className="px-6 py-4 bg-slate-900/40 border-y border-slate-700 flex-shrink-0">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div className="flex items-center space-x-2 text-slate-300">
              <Icon icon={gamemodeIcon} className="w-5 h-5 text-sky-400" />
        ...
        <div className="px-6 py-6 overflow-y-auto flex-grow">
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-slate-400">{strategy.authors.join(", ")}</p>
            </div>
                ...
                <div className="space-y-2">
                  {strategy.docs.map((doc) => (
                    <a
                      ...
                      className="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white px-4 py-3 rounded-lg font-medium transition-all duration-200 flex items-center space-x-3 shadow-lg hover:shadow-xl"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {strategy.povs.map((pov) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="bg-slate-700/50 hover:bg-slate-700 text-white px-4 py-3 rounded-lg font-medium transition-all duration-200 flex items-center space-x-3"
                    >
                      <Icon
                        icon="mdi:youtube"
